            ...
            renderFormItem: () => (
                <TreeSelect
                    ...
                    onChange={(value) => {
                        ...
                        // setDirectoryData({
                        //   // id: [directories.map(item => item.id)],
                        //   path: directories.map(item => item.path),
                        // });
    ...
    useEffect(() => {
        ...
        const fetchStandardMappingOptions = async () => {
            try {
                ...
                console.log('映射标准：', res);
                //只存id chName enName在standardMappingOptions里面，用于下拉选择,键为id的值
                const options = res.data.records.map((item: any) => ({
                    label: `${item.id} ${item.chName} ${item.enName}`,
                    value: item.id,
                }));
                setStandardMappingOptions(options);
                console.log('options', standardMappingOptions);
        ...
        if (isEdit && record) {
            ...
            form.setFieldsValue({
                ...
                dataFieldList: record.dataFieldList,
            });
            console.log('目录', record.directoryId);
            const paths = getDirectoryPaths(record.directoryId)
            console.log('pathhh', paths);

            //把directoryId转换为['directoryId']形式
            const directoryIds = record.directoryId.split(',');
            console.log('directoryIds111', directoryIds);
            setDirectoryData(directoryIds)

            console.log('directoryData', directoryData);
            getResourcesByIdsUsingGet({ ids: record.directoryId }).then(res => {//获取目录信息
                console.log('目录信息res', res);
                // const a = convertTreeData(res.data);
                // console.log('a', a);
                setDirectoryData(res.data);
                // setTreeData(a);
                console.log('treeData2', directoryData);
